...
class VideoTest(object):
    ...
    def run(self, video_path, start_frame = 0, conf_thresh = 0.6):
        ...
        out = cv2.VideoWriter('../result/result.avi',fourcc, 20.0, (int(self.input_shape[0]*vidar),self.input_shape[1]),True)
        ...
        while True:
            ...
            retval_2, orig_image_2= vid.read()
            if not retval_2:
                print("Done!")
                return
            im_size = (self.input_shape[0], self.input_shape[1])    
            resized = cv2.resize(orig_image, im_size)
            resized_2= cv2.resize(orig_image_2, im_size)
            ...
            to_draw_2= cv2.resize(resized_2, (int(self.input_shape[0]*vidar), self.input_shape[1]))
            ...
            if len(results) > 0 and len(results[0]) > 0:
                ...
                for i in range(top_conf.shape[0]):
                    ...
                    if(self.class_names[class_num]=="person"):
                        ...
                        cv2.rectangle(to_draw_2, (xmin, ymin), (xmax, ymax), 
                                      self.class_colors[class_num], 2)
                        ...
                        cv2.rectangle(to_draw_2, text_top, text_bot, self.class_colors[class_num], -1)
                        cv2.putText(to_draw_2, text, text_pos, cv2.FONT_HERSHEY_SIMPLEX, 0.35, (0,0,0), 1)
                out.write(to_draw)
                out.write(to_draw_2)
        out.release()
